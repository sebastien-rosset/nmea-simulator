...
class BasicNavSimulator:
    ...
    def __init__(self, udp_host="127.0.0.1", udp_port=10110):
        ...
        self.sog = 0.0  # Speed over ground in knots
    ...
    def _update_simulation_state(self, current_time: float, last_update: float) -> bool:
        ...
        if not self.route_manager.update_course_to_waypoint(current_pos):
            logging.info("Navigation complete or error")
            return False
            
        # Update vessel position and dynamics
    ...
    def _send_nmea_messages(self):
        ...
        self.message_service.send_gga(
            self.position, self.gps_quality, self.satellites_in_use,
            self.hdop, self.altitude, self.geoid_separation,
            self.dgps_age, self.dgps_station
        )
