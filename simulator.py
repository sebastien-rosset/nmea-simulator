...
class BasicNavSimulator:
    ...
    def update_rudder_angle(self, desired_heading: float, delta_time: float):
        ...
        desired_rudder = max(-self.max_rudder_angle, 
                           min(self.max_rudder_angle, desired_rudder))
        ...
        if abs(desired_rudder - current) <= max_change:
            ...
            self.starboard_rudder += max_change if desired_rudder > current else -max_change
    ...
    def send_rmb(self):
        ...
        from_waypoint = f"WP{self.current_waypoint_index-1:03d}" if self.current_waypoint_index > 0 else ""
    ...
    def update_position(self, delta_time):
        """Update position based on SOG, COG, and rudder angle"""
        # First update the rudder based on desired course
        self.update_rudder_angle(self.cog, delta_time)
        
        # Rudder effect on turn rate (simplified model)
        # Typical ship might turn 1 degree per second at full rudder
        ...
        # Original position update code
        speed_deg_per_sec = (self.sog / 60) * (delta_time / 3600)
        lat_change = speed_deg_per_sec * math.cos(math.radians(90 - self.cog))
        lon_change = speed_deg_per_sec * math.sin(math.radians(90 - self.cog)) / math.cos(math.radians(self.position['lat']))
        
        self.position['lat'] += lat_change
        self.position['lon'] += lon_change

    def simulate(self, waypoints: List[Dict[str, Union[str, float]]], duration_seconds: Optional[float] = None, update_rate: float = 1):
        ...
        try:
            while True:
                ...
                if duration_seconds is not None and (current_time - start_time) >= duration_seconds:
